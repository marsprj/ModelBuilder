 ...
 * 基于相关系数的变化检测
 */
var CorrelationDiffDetection = function(){

	FuncNode.apply(this, arguments);

	this._name = "CorrelationDiffDetection";
...
extend(CorrelationDiffDetection, FuncNode);


CorrelationDiffDetection.prototype.setParms = function(parms){
	if(!parms){
		return;
	}
...
CorrelationDiffDetection.prototype.export = function(){
	var obj = {
		id : this.getID(),
		name : this._name,
		inputs : [
...
CorrelationDiffDetection.prototype.onClick = function(isPathEdit){
	var inputs = [];
	var output;
	if(this._inputs){
	...
	var dlg = new CorrelationDiffDetectionDialog(inputs, output, parms, function(){	//onOK
...
CorrelationDiffDetection.prototype.updateParms = function(parms){
	if(!parms){
		return;
	}
