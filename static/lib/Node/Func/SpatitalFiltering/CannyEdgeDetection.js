var CannyEdgeDetection = function(){
	...
	this._name = "CannyEdgeDetection";

	this._inputsNumber = 1;
}


extend(CannyEdgeDetection, FuncNode);

CannyEdgeDetection.prototype.export = function(){
	var obj = {
		id : this.getID(),
		name : this._name,
		inputs : [
...
CannyEdgeDetection.prototype.onClick = function(isPathEdit){
	var inputs = [];
	var output;
	$(".tooltip").remove();
	if(this._inputs){
	...
	var parms = [];
	var dlg = new CannyEdgeDetectionDialog(inputs, output,parms, function(){	//onOK
