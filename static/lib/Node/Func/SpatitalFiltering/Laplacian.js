...
extend(Laplacian, FuncNode);

Laplacian.prototype.export = function(){
	var obj = {
		id : this.getID(),
		name : this._name,
		inputs : [
...
Laplacian.prototype.onClick = function(isPathEdit){
	var inputs = [];
	var output;
	$(".tooltip").remove();
	if(this._inputs){
	...
	var parms = [];
	var dlg = new LaplacianDialog(inputs, output,parms, function(){	//onOK
