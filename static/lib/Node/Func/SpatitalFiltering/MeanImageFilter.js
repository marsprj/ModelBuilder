 ...
 */
var MeanImageFilter  = function(){
	FuncNode.apply(this, arguments);

	this._name = "MeanImageFilter";

	this._inputsNumber = 1;
}

extend(MeanImageFilter, FuncNode);


MeanImageFilter.prototype.export = function(){
	var obj = {
		id : this.getID(),
		name : this._name,
		inputs : [
...
MeanImageFilter.prototype.onClick = function(isPathEdit){
	var inputs = [];
	var output;
	$(".tooltip").remove();
	if(this._inputs){
	...
	var parms = [];
	var dlg = new MeanDialog(inputs, output,parms, function(){	//onOK
